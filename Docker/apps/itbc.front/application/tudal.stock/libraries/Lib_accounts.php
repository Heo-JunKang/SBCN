...
class Lib_accounts {
	...
	public function set_uid($reponse) {
		...
		if($response->response->code==201) {
			...
			$user_id	= $response->contents->items->user_id;
			$site_code	= $response->contents->items->site_code;
			$user_no	= $response->contents->items->user_no;
			$user_name	= $response->contents->items->user_name;
			$phone		= $response->contents->items->phone;
			$api_key	= $response->contents->items->api_key;
			...
			$user_status= $response->contents->items->user_status_code == 304 ? 'vip' : 'free';

			$uid	= $user_id.'|'.$user_no.'|'.$user_name.'|'.$phone.'|'.$site_code.'|'.$api_key.'|'.$user_status;
			$uid 	= base64_encode($uid);
			$expire	= 3600*24*365;

			set_cookie('itbc_uid',$uid,$expire,config_item('cookie_domain'),'/');
		}
	}
	...
	public function logout() {
		delete_cookie('itbc_uid',config_item('cookie_domain'),'/');
	}
