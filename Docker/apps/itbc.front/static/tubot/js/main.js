            ...
            if(rows!=null){
                ...
                html += '<li class="catch-ev-li signalbox02_li" value="'+row.shcode+'"> <div class="left-con"><p class="txt">'+(idx+1)+'위. '+row.shname+'</p><ul class="number-ico-list">';
                    ...
                    $.each(signal_array, function(id,signal){
                        html += '<li><img src="/theme/tubot/source_dev/img/content/img_signal_'+signal+'.png" alt=""></li>';
                    });

                html += '</ul></div><div class="right-con">';
                html += '<p class="txt signalbox2_pr" id="signalbox02_pr'+idx+'" style="line-height:45px;"></p></div></li>';
...
function updateStockData(shcode, idx){
    $.ajax({
        type:"POST",
        url: "/action/redis.php",
        data : {"mode":"sise", "shcode": shcode},
        dataType:"json",
        ...
        success:function(data, textStatus, jqXHR) {
			...
			if( (parseFloat(data.data.diff).toFixed(2)) >= 0 ) {
                html = '+'+parseFloat(data.data.diff).toFixed(2)+'%<br><span></span>';
            } else if ( (parseFloat(data.data.diff).toFixed(2)) < 0 ) {
                html = '<span style="color:blue;font-size:16px;">'+parseFloat(data.data.diff).toFixed(2)+'%<br></span><span></span>';
            }
...
function updateStockDatas(){
    $(".signalbox02_li").each(function(idx) {
        var shcode = $(this).attr('value').pad(6);
...
function startWorker() {
    ...
    if ( !!window.Worker ) {
        ...
        //loading
        worker = new Worker( '../theme/tubot/js/worker.js' );
